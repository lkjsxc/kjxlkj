...
#[actix_web::test]
async fn admin_routes_redirect_by_setup_and_session_state() {
    ...
    assert_eq!(before_setup.status(), StatusCode::FOUND);
    assert_eq!(
        before_setup
            .headers()
            .get(header::LOCATION)
    ...
    let without_session =
        test::call_service(&app, test::TestRequest::get().uri("/admin").to_request()).await;
    assert_eq!(without_session.status(), StatusCode::FOUND);
    assert_eq!(
        without_session
            .headers()
            .get(header::LOCATION)
    ...
    let with_session = test::call_service(
        &app,
