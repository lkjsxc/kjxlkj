# TODO: Features (Editing Helpers)

Back: [/docs/todo/current/README.md](/docs/todo/current/README.md)

## Defining specs

- [/docs/spec/features/editing/README.md](/docs/spec/features/editing/README.md)

## Editing helper features

- [ ] Auto-pairs per [/docs/spec/features/editing/auto-pairs.md](/docs/spec/features/editing/auto-pairs.md)
- [ ] Clipboard integration per [/docs/spec/features/editing/clipboard.md](/docs/spec/features/editing/clipboard.md)
- [ ] Comment toggling per [/docs/spec/features/editing/comments.md](/docs/spec/features/editing/comments.md)
- [ ] Editing helpers per [/docs/spec/features/editing/editing_helpers.md](/docs/spec/features/editing/editing_helpers.md)
- [ ] Format on paste per [/docs/spec/features/editing/format-on-paste.md](/docs/spec/features/editing/format-on-paste.md)
- [ ] Format on type per [/docs/spec/features/editing/format-on-type.md](/docs/spec/features/editing/format-on-type.md)
- [ ] Multi-cursor per [/docs/spec/features/editing/multicursor.md](/docs/spec/features/editing/multicursor.md)
- [ ] Snippets per [/docs/spec/features/editing/snippets.md](/docs/spec/features/editing/snippets.md)
- [ ] Spell checking per [/docs/spec/features/editing/spell.md](/docs/spec/features/editing/spell.md)
- [ ] Surround per [/docs/spec/features/editing/surround.md](/docs/spec/features/editing/surround.md)
- [ ] Templates per [/docs/spec/features/editing/templates.md](/docs/spec/features/editing/templates.md)

## UI features

- [/docs/spec/features/ui/README.md](/docs/spec/features/ui/README.md)

### Viewport

- [ ] Line wrapping with CJK boundary padding per [/docs/spec/features/ui/viewport.md](/docs/spec/features/ui/viewport.md)
- [ ] Long line overflow wraps to next display row (REG-02)
- [ ] Width-2 grapheme at wrap boundary produces padding cell (REG-08)
- [ ] Horizontal/vertical follow cursor

### Statusline

- [/docs/spec/features/ui/statusline/README.md](/docs/spec/features/ui/statusline/README.md)
- [ ] Statusline rendering per [/docs/spec/features/ui/statusline/statusline.md](/docs/spec/features/ui/statusline/statusline.md)
- [ ] Statusline DSL per [/docs/spec/features/ui/statusline/statusline-dsl.md](/docs/spec/features/ui/statusline/statusline-dsl.md)
- [ ] Statusline config per [/docs/spec/features/ui/statusline/statusline-config.md](/docs/spec/features/ui/statusline/statusline-config.md)

### UI details

- [ ] Cursor customization per [/docs/spec/features/ui/cursor-customization.md](/docs/spec/features/ui/cursor-customization.md)
- [ ] Indent guides per [/docs/spec/features/ui/indent-guides.md](/docs/spec/features/ui/indent-guides.md)
- [ ] Notifications per [/docs/spec/features/ui/notifications.md](/docs/spec/features/ui/notifications.md)
- [ ] Popup API per [/docs/spec/features/ui/popup-api.md](/docs/spec/features/ui/popup-api.md)
- [ ] Scroll customization per [/docs/spec/features/ui/scroll-customization.md](/docs/spec/features/ui/scroll-customization.md)
- [ ] Icons per [/docs/spec/features/ui/icons.md](/docs/spec/features/ui/icons.md)
- [ ] Color picker per [/docs/spec/features/ui/color-picker.md](/docs/spec/features/ui/color-picker.md)
- [ ] Font rendering per [/docs/spec/features/ui/font-rendering.md](/docs/spec/features/ui/font-rendering.md)
- [ ] Ligatures per [/docs/spec/features/ui/ligatures.md](/docs/spec/features/ui/ligatures.md)
