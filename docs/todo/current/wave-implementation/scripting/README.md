...
- [ ] Implement the full mapping and automation spec subtree. — done: mapping_expansion.rs (recursive expansion), event_automation.rs (event dispatch), script_loader.rs (file parsing)
- [ ] Ensure recursion/plug mappings are handled exactly as specified (or recorded as limitations). — done: `mapping_expansion.rs` MAX_DEPTH=100 recursion guard, prefix match
