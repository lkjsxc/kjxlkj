...
- [ ] Placeholder scaffolding: define completion provider interfaces. — done: `CompletionProvider`, `CompletionProviderKind`, `CompletionRegistry` in `scripting.rs`
...
- [ ] Full conformance: implement all completion behaviors described by the spec. — done: `completion_engine.rs` with CompletionSource (Command/Path/Option/Buffer/Help/ColorScheme/Custom), CompletionItem, CompletionState (next/prev/current/reset), detect_source, complete_commands, complete_paths, common_prefix
