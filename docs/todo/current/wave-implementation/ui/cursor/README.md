# UI: Cursor Rendering (Iteration 34)

Back: [/docs/todo/current/wave-implementation/ui/README.md](/docs/todo/current/wave-implementation/ui/README.md)

## Scope

Implement cursor rendering rules, including interaction with selection, highlights, and overlays.

## Defining documents (direct, normative)

- Cursor:
  - [/docs/spec/editing/cursor/README.md](/docs/spec/editing/cursor/README.md)
- Cursor customization:
  - [/docs/spec/features/ui/cursor-customization.md](/docs/spec/features/ui/cursor-customization.md)

## Checklist

- [ ] Placeholder scaffolding: define cursor shape/visibility in snapshots.
- [ ] Minimal slice: ensure cursor is always visible across mode transitions and redraws.
- [ ] Full conformance: implement all cursor rendering and customization behaviors.
  - cursor_rendering.rs: CursorShape (Block/Line/Underline/Hidden), BlinkState, CursorConfig, ModeCursorMap (shape_for_mode), cursor_shape_escape, blink_state, cursor_visible

