# Command Execution

Command execution modes and mechanisms.

## Documents

| Document | Content |
|----------|---------|
| [execute-command.md](execute-command.md) | Execute command |
| [normal-command.md](normal-command.md) | Normal mode commands |
| [source-command.md](source-command.md) | Sourcing script files |

## Related

- Commands overview: [docs/spec/commands/README.md](docs/spec/commands/README.md)
- Scripting: [docs/spec/scripting/README.md](docs/spec/scripting/README.md)
