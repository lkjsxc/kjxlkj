# Visual Mode Editing

Visual mode editing specifications.

## Documents

| Document | Content |
|----------|---------|
| [visual-selections.md](visual-selections.md) | Visual selections |
| [blockwise-visual.md](blockwise-visual.md) | Blockwise visual |
| [column-block.md](column-block.md) | Column block editing |

## Related

- Editing overview: [docs/spec/editing/README.md](docs/spec/editing/README.md)
- Modes visual: [docs/spec/modes/visual.md](docs/spec/modes/visual.md)
