...
|---|---|---|---|---|---|---|---|
| `R-BASELINE-01` | [/docs/spec/architecture/workspace-manifest.md](/docs/spec/architecture/workspace-manifest.md) | grouped workspace and crate tree exist | topology + build gate | verified | closed | implement | 20-crate workspace, `cargo check --workspace` passes, 295 tests pass (2026-02-11) |
...
| `R-EXP-01` | [/docs/spec/features/navigation/file_explorer.md](/docs/spec/features/navigation/file_explorer.md) | `:Explorer` and leader routes are user-visible and reliable | `EXP-01R`..`EXP-06R` | partial | `M2`, `M4` | implement + test-add | explorer open/close routing implemented (ContentKind::Explorer leaf creation, :ExplorerClose ex command), 6 integration tests; state model/tree data/key navigation pending |
