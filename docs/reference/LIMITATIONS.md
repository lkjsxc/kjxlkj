...
| LIM-OPMOTION-01 | Operator+motion composition (d2w, c$, etc.) | Operator+motion, doubled operators (dd/cc/yy/>>/<<), count multiplication, gg motion, f/t/F/T motions in op-pending + ;/, repeat, text objects (iw/aw/ip/ap/iW/aW + delimiter objects), mark motions ('/`) in op-pending all implemented | `partial` | `/docs/spec/editing/operators/README.md` | Additional text objects (is/as, it/at) |
| LIM-CMDCOMPL-01 | Command-line Tab completion | Tab/Shift-Tab/BackTab cycle through prefix-matched command name candidates; file-path completion after command+space using read_dir | `partial` | `/docs/spec/commands/cmdline/completion.md` | Add option and buffer completion contexts |
...
| LIM-MARKS-01 | Local and global marks | MarkFile supports local, global, special marks; m{char} sets marks, '{char}/`{char} jumps to marks; mark motions wired in operator-pending mode; special marks: . and ^ set on Insert→Normal, < and > set on Visual→Normal, [ and ] setter methods available | `partial` | `/docs/spec/editing/marks/README.md` | Wire [ ] marks into change operations |
| LIM-MACROS-01 | Macro recording and playback | q{a-z} records, q stops, @{a-z} plays back, @@ replays last; q{A-Z} appends; :registers displays macro content with key-to-char conversion | `partial` | `/docs/spec/editing/macros/README.md` | Full unification of macro store with register file |
...
| LIM-RANGEPATTERN-01 | Pattern-based ranges (/pattern/, ?pattern?) and mark-based ranges | /pattern/, ?pattern?, '{mark} ranges with offsets, \/ and \? last-search shorthand, semicolon (;) range separator all implemented | `partial` | `/docs/spec/commands/ranges/ranges.md` | Support visual range ('<,'>) shorthand |
