...
| LIM-CMDCOMPL-01 | Command-line Tab completion | Tab/Shift-Tab/BackTab cycle through prefix-matched candidates with fuzzy fallback; file-path, option-name (:set), buffer-name (:b), and user-defined command completion wired; wildmenu renders completions on status bar row with highlight | `partial` | `/docs/spec/commands/cmdline/completion.md` | Wildmenu scrolling for large lists |
| LIM-REGISTERS-01 | Named registers (a-z, A-Z, 0-9, etc.) | RegisterFile supports named, numbered, small-delete, clipboard, black-hole, expression (=), last-inserted (.), read-only (% # : /); Ctrl-R in insert mode inserts register content; Ctrl-R = evaluates arithmetic/string expressions via expr_eval; put and :registers display | `partial` | `/docs/spec/editing/registers/README.md` | Full expression prompt with = cmdline |
