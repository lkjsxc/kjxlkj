...
| LIM-VISUAL-01 | Visual mode selection and operations | Visual char/line/block selection with motions, operators (d/c/y/>/<), o-swap; Ctrl-V enters block mode; I/A in block mode inserts on each line; block change replicates text to all lines; visual p/P paste with block column-wise paste support; r{char} block/char/line replace; * and # search for selected text; gv reselects last visual area; visual selection rendered with column highlighting (blue bg) in grid | `partial` | `/docs/spec/modes/visual.md` | Visual J join, = reindent |
...
| LIM-REGISTERS-01 | Named registers (a-z, A-Z, 0-9, etc.) | RegisterFile supports named, numbered, small-delete, clipboard, black-hole, expression (=), last-inserted (.), read-only (% # : /); Ctrl-R in insert mode inserts register content; Ctrl-R = opens expression mini-cmdline with arithmetic/string evaluation and g:/b:/w:/v: variable references; put and :registers display | `partial` | `/docs/spec/editing/registers/README.md` | Expression function calls |
