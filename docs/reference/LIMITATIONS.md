...
| LIM-OPMOTION-01 | Operator+motion composition (d2w, c$, etc.) | Operator+motion, doubled operators (dd/cc/yy/>>/<<), count multiplication, gg motion, f/t/F/T motions in op-pending + ;/,  repeat implemented; text objects not yet wired | `partial` | `/docs/spec/editing/operators/README.md` | Wire text objects (iw/aw/ip/ap) |
...
| LIM-REGISTERS-01 | Named registers (a-z, A-Z, 0-9, etc.) | RegisterFile supports named, numbered, small-delete, clipboard, black-hole; " prefix selects register; yank/delete/put operations use pending_register or unnamed | `partial` | `/docs/spec/editing/registers/README.md` | Support numbered register rotation on delete |
...
| LIM-TEXTMANIP-01 | Text manipulation (indent, format, case, etc.) | Indent/dedent, ~ toggle-case, gu{motion} lowercase, gU{motion} uppercase implemented; format operator not yet wired | `partial` | `/docs/spec/editing/text-manipulation/README.md` | Implement format operator (gq) |
