...
| LIM-VISUAL-01 | Visual mode selection and operations | Visual char/line/block selection with motions, operators (d/c/y/>/<), o-swap; Ctrl-V enters block mode; I/A in block mode inserts on each line; block change replicates text to all lines; visual p/P paste with block column-wise paste support; r{char} block/char/line replace; * and # search for selected text; gv reselects last visual area; J joins selected lines; = reindents selection; u lowercases and U uppercases selection; visual selection rendered with column highlighting (blue bg) in grid | `partial` | `/docs/spec/modes/visual.md` | Visual g? ROT13, ~ toggle case |
