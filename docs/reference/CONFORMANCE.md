...
- Structured tracing with request_id, method, path, latency per request.
- CSP nonce strategy for script execution hardening.
- Graceful shutdown with SIGTERM/Ctrl-C support.
- Cross-session WebSocket broadcast registry.
...
- SQLx offline cache infrastructure (.sqlx/ directory).
- E2E acceptance tests covering all mandatory acceptance IDs.
- All source files verified ≤ 200 lines.
...
| Runtime crates and services | [/docs/spec/architecture/runtime.md](/docs/spec/architecture/runtime.md) | `verified` | 10 crates compile, 152 tests pass, 0 warnings |
...
| Editor and responsive UX | [/docs/spec/ui/editor-flow.md](/docs/spec/ui/editor-flow.md) | `verified` | autosave+409 retry, keyboard shortcuts, wiki-link extraction, markdown preview (split/toggle), backlinks, conflict UX, note-list with create+search, compact layout, 80+ TS assertions |
...
| Auth/session/CSRF | [/docs/spec/security/README.md](/docs/spec/security/README.md) | `verified` | session cookie, CSRF validation, argon2 hashing, setup lock, logout cookie clear |
