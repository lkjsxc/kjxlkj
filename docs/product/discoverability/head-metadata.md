...
- HTML pages may emit a canonical link only when `PUBLIC_BASE_URL` is configured and valid.
...
- Homepage is `index,follow` only when `PUBLIC_BASE_URL` is configured and valid.
- Public live note pages are `index,follow` only when `PUBLIC_BASE_URL` is configured and valid.
- Search, setup, login, logout-result pages, admin, settings, history, snapshots, and not-found pages are `noindex,nofollow`.
- When `PUBLIC_BASE_URL` is missing or invalid, every HTML page becomes `noindex,nofollow`.
