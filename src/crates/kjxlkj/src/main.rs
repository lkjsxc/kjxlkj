...
async fn run(args: cli::Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (action_tx, mut action_rx) = mpsc::channel::<kjxlkj_core_types::Action>(256);
    let (key_tx, mut key_rx) = mpsc::channel::<kjxlkj_core_types::Key>(256);
    let (response_tx, mut response_rx) = mpsc::channel::<kjxlkj_core_types::ServiceResponse>(256);

    // Get initial terminal size.
    let (init_cols, init_rows) = crossterm::terminal::size().unwrap_or((80, 24));

    // Editor state.
    let mut editor = kjxlkj_core::state::EditorState::new(init_cols, init_rows);

    // Load files from CLI.
    for file in &args.files {
        if let Ok(content) = tokio::fs::read_to_string(file).await {
            let buf_id = editor.alloc_buffer_id();
            let buf = kjxlkj_core::state::BufferState::from_content(buf_id, file.clone(), &content);
    ...
    // Signals.
    let signal_handler = kjxlkj_host::SignalHandler::new(quit_tx.clone());
    ...
    #[cfg(unix)]
    {
        ...
        tokio::spawn(kjxlkj_host::watch_sigwinch(action_tx_sig));
    }

    // Services.
    let mut supervisor = kjxlkj_services::ServiceSupervisor::new(response_tx, quit_tx.clone());
    supervisor.start_all();

    // Input reader.
    let quit_rx_input = quit_tx.subscribe();
    let input_reader = kjxlkj_input::InputReader::new(action_tx.clone(), key_tx);
    tokio::spawn(async move {
        input_reader.run(quit_rx_input).await;
    });

    // Renderer.
    let mut renderer = kjxlkj_render::Renderer::new(init_cols, init_rows);
