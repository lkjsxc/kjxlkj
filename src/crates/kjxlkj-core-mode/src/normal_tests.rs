...
mod tests {
    ...
    use kjxlkj_core_types::{Action, InsertPosition, Key, Motion, Operator};
    ...
    fn count_accumulation() {
        ...
        assert!(matches!(a, Some(Action::MoveCursor(Motion::Down, 3))));
    }
    ...
    fn find_char_forward() {
        let mut s = NormalModeState::new();
        assert!(s.process_key(&Key::char('f')).is_none());
        let a = s.process_key(&Key::char('x'));
        assert!(matches!(
            a,
            Some(Action::MoveCursor(Motion::FindCharForward('x'), 1))
        ));
    }
    ...
    fn ctrl_w_h_focuses_left() {
        ...
        assert!(matches!(
            a,
            Some(Action::FocusWindow(kjxlkj_core_types::Direction::Left))
        ));
    }
    ...
    fn bracket_match_key() {
        ...
        assert!(matches!(
            a,
            Some(Action::MoveCursor(Motion::MatchingBracket, 1))
        ));
    }
}
