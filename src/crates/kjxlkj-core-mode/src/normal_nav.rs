    ...
    pub(crate) fn try_dispatch_nav(
        ...
        match (&key.code, key.modifiers) {
            (KeyCode::Char('n'), KeyModifiers::NONE) => {
                Some(Some(Action::NextMatch))
            }
            (KeyCode::Char('N'), KeyModifiers::NONE) => {
                Some(Some(Action::PrevMatch))
            }
            ...
            (KeyCode::Char('f'), KeyModifiers::NONE) => {
                self.find_char_pending =
                    Some(FindCharPending::Forward);
                Some(None)
            }
            (KeyCode::Char('F'), KeyModifiers::NONE) => {
                self.find_char_pending =
                    Some(FindCharPending::Backward);
                Some(None)
            }
            (KeyCode::Char('t'), KeyModifiers::NONE) => {
                self.find_char_pending =
                    Some(FindCharPending::TillForward);
                Some(None)
            }
            (KeyCode::Char('T'), KeyModifiers::NONE) => {
                self.find_char_pending =
                    Some(FindCharPending::TillBackward);
                Some(None)
            }
            // Scroll motions
            (KeyCode::Char('d'), m)
