...
use kjxlkj_core_types::{Action, Key, KeyCode, KeyModifiers, Operator, VisualKind};

use crate::visual::VisualModeState;

impl VisualModeState {
    /// Process operator/command keys in visual mode.
    pub(crate) fn dispatch_visual_command(&mut self, key: &Key) -> Option<Action> {
        let action = match (&key.code, key.modifiers) {
            ...
            (KeyCode::Char('d'), KeyModifiers::NONE) | (KeyCode::Char('x'), KeyModifiers::NONE) => {
                self.reset_count();
                return Some(Action::Delete(kjxlkj_core_types::Motion::Right, 1));
            }
            (KeyCode::Char('c'), KeyModifiers::NONE) | (KeyCode::Char('s'), KeyModifiers::NONE) => {
                self.reset_count();
                return Some(Action::Change(kjxlkj_core_types::Motion::Right, 1));
            }
            (KeyCode::Char('y'), KeyModifiers::NONE) => {
                self.reset_count();
                return Some(Action::Yank(kjxlkj_core_types::Motion::Right, 1));
            }
            ...
            (KeyCode::Char('U'), KeyModifiers::NONE) => {
                self.reset_count();
                return Some(Action::DoubleOperator(Operator::Uppercase, 1));
            }
            (KeyCode::Char('u'), KeyModifiers::NONE) => {
                self.reset_count();
                return Some(Action::DoubleOperator(Operator::Lowercase, 1));
            }
