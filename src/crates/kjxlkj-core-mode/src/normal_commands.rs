...
use kjxlkj_core_types::{Action, InsertPosition, Key, KeyCode, KeyModifiers, Operator, VisualKind};

use crate::normal::NormalModeState;

impl NormalModeState {
    pub(crate) fn dispatch_key_commands(&mut self, key: &Key, count: u32) -> Option<Action> {
        let action = match (&key.code, key.modifiers) {
            ...
            // --- Visual mode ---
            (KeyCode::Char('v'), KeyModifiers::NONE) => Action::EnterVisual(VisualKind::Char),
            (KeyCode::Char('V'), KeyModifiers::NONE) => Action::EnterVisual(VisualKind::Line),
            ...
            _ => {
                return self.dispatch_key_single(key, count);
            }
        };
