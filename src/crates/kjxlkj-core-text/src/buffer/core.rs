...
impl TextBuffer {
    ...
    pub fn char_count(&self) -> usize { self.rope.len_chars() }
    ...
    pub fn delete_range(&mut self, start: Position, end: Position) {
        if let (Some(s), Some(e)) = (self.pos_to_char_idx(start), self.pos_to_char_idx(end)) {
    ...
    pub fn pos_to_char_idx(&self, pos: Position) -> Option<usize> {
        if pos.line >= self.line_count() { return None; }
        let line_start = self.rope.line_to_char(pos.line);
        let col = pos.col.min(self.line_len(pos.line));
        Some(line_start + col)
    }
