...
//!
//! Covers: explorer state lifecycle, explorer key dispatch through
//! handle_key(), explorer + wincmd interaction, terminal state model.
...
/// Set up an explorer with a synthetic tree and expansion.
fn setup_explorer(s: &mut EditorState) {
    ...
    let mut src = ExplorerNode::dir(
        ids[1], "src".into(), 1, PathBuf::from("/proj/src"),
    );
    src.children.push(ExplorerNode::file(
        ids[3], "main.rs".into(), 2, PathBuf::from("/proj/src/main.rs"),
    ));
    src.children.push(ExplorerNode::file(
        ids[4], "lib.rs".into(), 2, PathBuf::from("/proj/src/lib.rs"),
    ));
    let root = ExplorerNode {
        ...
        children: vec![
            src,
            ExplorerNode::file(
                ids[2], "README.md".into(), 1, PathBuf::from("/proj/README.md"),
            ),
        ],
    };
...
#[test]
fn explorer_states_populated_on_open() {
...
#[test]
fn explorer_states_cleaned_on_close() {
    let mut s = EditorState::new(80, 24);
    s.open_explorer();
    assert_eq!(s.explorer_states.len(), 1);
    s.close_explorer();
...
#[test]
fn explorer_reopen_reuses_window() {
    let mut s = EditorState::new(80, 24);
    s.open_explorer();
    let first_eid = s.focused_explorer_id().unwrap();
    wincmd(&mut s, 'w'); // switch away
    s.open_explorer(); // should focus existing
    let second_eid = s.focused_explorer_id().unwrap();
    assert_eq!(first_eid, second_eid);
    assert_eq!(s.explorer_states.len(), 1);
}

// --- Explorer key dispatch through handle_key ---

#[test]
fn handle_key_j_moves_explorer_selection() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
    let eid = s.focused_explorer_id().unwrap();
    assert_eq!(s.explorer_states[&eid].selected_index, 0);
    // 'j' through full handle_key dispatch
    let (k, m) = key('j');
    press(&mut s, &k, &m);
...
#[test]
fn handle_key_k_moves_explorer_up() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
    let eid = s.focused_explorer_id().unwrap();
    let (jk, jm) = key('j');
...
fn handle_key_l_expands_dir() {
    ...
    let (lk, lm) = key('l');
    press(&mut s, &lk, &lm);
    // Should now show: proj, src, main.rs, lib.rs, README.md = 5
    assert_eq!(s.explorer_states[&eid].row_count(), 5);
}

#[test]
fn handle_key_h_collapses_dir() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
    let eid = s.focused_explorer_id().unwrap();
    // Expand src first
...
#[test]
fn handle_key_q_closes_explorer_via_dispatch() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
...
#[test]
fn handle_key_enter_expands_like_l() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
    let eid = s.focused_explorer_id().unwrap();
    let (jk, jm) = key('j');
...
// --- Explorer + wincmd interaction ---

#[test]
fn wincmd_works_from_explorer_window() {
    let mut s = EditorState::new(80, 24);
    setup_explorer(&mut s);
    let exp_wid = s.focus.focused;
    // Ctrl-w w should cycle to buffer window
    wincmd(&mut s, 'w');
...
#[test]
fn buffer_keys_unaffected_without_explorer() {
    let mut s = EditorState::new(80, 24);
    // 'j' in normal mode on buffer should move cursor, not trigger explorer
    let (jk, jm) = key('j');
    press(&mut s, &jk, &jm);
    // Should not crash; cursor moves (or stays if single line)
...
#[test]
fn terminal_state_default_size() {
    use kjxlkj_core_types::TerminalId;
    use kjxlkj_service_terminal::TerminalState;
