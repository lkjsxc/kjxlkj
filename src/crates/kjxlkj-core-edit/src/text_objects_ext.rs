...
impl TextRange {
    ...
    pub fn contains(&self, pos: Position) -> bool { pos >= self.start && pos < self.end }
...
pub fn find_argument(lines: &[&str], pos: Position, inner: bool) -> Option<Range> {
    if pos.line >= lines.len() { return None; }
    ...
    loop {
        ...
        if matches!(c, '(' | '[' | '{') {
            if depth > 0 { depth -= 1; } else { start += 1; break; }
        }
        if c == ',' && depth == 0 { start += 1; break; }
        if start == 0 { break; }
        start -= 1;
    }
    depth = 0;
    ...
    while end < ch.len() {
        ...
        if c == ',' && depth == 0 { if !inner { end += 1; } break; }
        end += 1;
    }
    if inner {
        while start < end && ch.get(start).map_or(false, |c| c.is_whitespace()) { start += 1; }
        while end > start && ch.get(end - 1).map_or(false, |c| c.is_whitespace()) { end -= 1; }
    }
    if start >= end { return None; }
    Some(Range::new(Position::new(pos.line, start), Position::new(pos.line, end)))
}
...
pub fn find_indent_level(lines: &[&str], pos: Position, inner: bool) -> Option<Range> {
    ...
    let mut s = pos.line;
    while s > 0 && (lines[s-1].trim().is_empty() || leading_ws(lines[s-1]) >= base) { s -= 1; }
    let mut e = pos.line;
    while e + 1 < lines.len() && (lines[e+1].trim().is_empty() || leading_ws(lines[e+1]) >= base) { e += 1; }
    if inner {
        while s < e && lines[s].trim().is_empty() { s += 1; }
        while e > s && lines[e].trim().is_empty() { e -= 1; }
    }
    Some(Range::new(Position::new(s, 0), Position::new(e, lines[e].len())))
}

/// Select the entire buffer.
pub fn find_entire_buffer(line_count: usize, _inner: bool) -> Range {
    if line_count == 0 { return Range::new(Position::ZERO, Position::ZERO); }
...
pub fn find_number(line: &str, col: usize) -> Option<Range> {
    ...
    if col >= ch.len() { return None; }
    if !ch[col].is_ascii_digit() && !matches!(ch[col], '-' | 'x' | 'X') { return None; }
    let mut s = col;
    while s > 0 && is_num(ch[s - 1]) { s -= 1; }
    if s > 0 && ch[s - 1] == '-' { s -= 1; }
    let mut e = col;
    while e + 1 < ch.len() && is_num(ch[e + 1]) { e += 1; }
    Some(Range::new(Position::new(0, s), Position::new(0, e + 1)))
}

fn is_num(c: char) -> bool { c.is_ascii_hexdigit() || matches!(c, 'x' | 'X' | 'o' | 'b') }
fn leading_ws(line: &str) -> usize { line.chars().take_while(|c| c.is_whitespace()).count() }
...
mod tests {
    ...
    #[test]
    fn find_number_basic() { assert!(find_number("abc 123 def", 5).is_some()); }
    #[test]
