...
pub(crate) fn move_word_forward(
    ...
    if let Some(next) =
        kjxlkj_core_text::find_word_boundary(
            &line,
    ...
    {
        let gc =
            content.line_graphemes(cursor.line).count();
        if next < gc {
...
pub(crate) fn move_paragraph_forward(
    cursor: &mut CursorPosition,
    content: &BufferContent,
) {
    ...
    while line < lc
        && !content.line_content(line).is_empty()
    {
        line += 1;
    }
    while line < lc
...
pub(crate) fn move_paragraph_backward(
    cursor: &mut CursorPosition,
    content: &BufferContent,
) {
    if cursor.line == 0 {
...
pub(crate) fn move_to_first_non_blank(
    cursor: &mut CursorPosition,
    content: &BufferContent,
) {
    let line = content.line_content(cursor.line);
