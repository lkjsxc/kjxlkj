...
pub(crate) fn exec_first_non_blank(
    cursor: &mut CursorPosition,
...
pub(crate) fn exec_last_non_blank(
    cursor: &mut CursorPosition,
    content: &BufferContent,
) {
    let line = content.line_content(cursor.line);
    let lg =
        kjxlkj_core_text::LineGraphemes::from_str(&line);
    let mut idx =
        if lg.count() > 0 { lg.count() - 1 } else { 0 };
...
pub(crate) fn exec_goto_first_line(
    cursor: &mut CursorPosition,
) {
    cursor.line = 0;
    cursor.grapheme_offset = 0;
...
pub(crate) fn exec_goto_last_line(
    cursor: &mut CursorPosition,
    line_count: usize,
) {
    cursor.line =
        if line_count > 0 { line_count - 1 } else { 0 };
    cursor.grapheme_offset = 0;
    cursor.clear_desired_col();
}

pub(crate) fn exec_goto_line(
    cursor: &mut CursorPosition,
    n: usize,
    line_count: usize,
) {
...
pub(crate) fn exec_goto_column(
    ...
    let gc = content.line_graphemes(cursor.line).count();
    cursor.grapheme_offset =
        col.min(if gc > 0 { gc - 1 } else { 0 });
    cursor.clear_desired_col();
}

/// Apply desired column after vertical movement.
pub(crate) fn apply_desired_col(
    cursor: &mut CursorPosition,
    content: &BufferContent,
) {
