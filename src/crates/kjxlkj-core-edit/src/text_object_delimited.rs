...
pub(crate) fn resolve_bracket(
    ...
    let (start_line, start_col) =
        find_open_bracket(cursor, content, open, close)?;
    let (end_line, end_col) = find_close_bracket(
        start_line,
        start_col,
        content,
        open,
    ...
    let (s, e) = match scope {
        TextObjectScope::Inner => {
            let mut s = CursorPosition::new(
                start_line,
                start_col + 1,
            );
            let e = CursorPosition::new(
                end_line,
...
pub(crate) fn resolve_paragraph(
    ...
    while start > 0
        && !content.line_content(start).is_empty()
    {
        start -= 1;
    }
    if content.line_content(start).is_empty()
        && start < cursor.line
    {
        start += 1;
    }
    ...
    while end + 1 < lc
        && !content.line_content(end).is_empty()
    {
        end += 1;
    }
