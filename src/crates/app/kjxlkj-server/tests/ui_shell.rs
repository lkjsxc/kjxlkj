...
async fn root_serves_workspace_shell_markup() {
    ...
    assert!(body.contains("id=\"librarianProvider\""));
    assert!(body.contains("id=\"librarianModel\""));
    assert!(body.contains("id=\"librarianRulesList\""));
    assert!(body.contains("id=\"librarianRunsList\""));
    assert!(body.contains("id=\"runReviewOps\""));
    assert!(body.contains("id=\"applyRunDecisions\""));
    ...
    assert!(body.contains("run-rule <id|selected> | review-run <id|selected>"));
    assert!(body.contains("Resolve local draft changes before apply/reject."));
