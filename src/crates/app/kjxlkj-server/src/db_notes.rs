...
pub async fn create_note(
    ...
    sqlx::query("insert into note_streams (id, title, current_version) values ($1, $2, 1)")
        .bind(note_id)
        .bind(title)
        .execute(&mut *tx)
        .await?;
    ...
    sqlx::query(
        "insert into note_projections (note_id, title, version, markdown, metadata_json, tags, search_vector)
         values ($1, $2, 1, $3, $4, $5, to_tsvector('simple', $2 || ' ' || $3))",
    )
    .bind(note_id)
    .bind(title)
    .bind(markdown)
...
pub async fn list_notes(
    ...
    let rows = sqlx::query_as::<_, NoteSummary>(
        "select id, title, current_version, updated_at, deleted_at
         from note_streams
...
pub async fn get_note(pool: &PgPool, note_id: Uuid) -> Result<NoteProjection, AppError> {
    let row = sqlx::query_as::<_, NoteProjection>(
        "select note_id, title, version, markdown, rendered_html, metadata_json, tags, updated_at
...
pub async fn backlinks(pool: &PgPool, note_id: Uuid) -> Result<Vec<NoteSummary>, AppError> {
    let rows = sqlx::query_as::<_, NoteSummary>(
        "select s.id, s.title, s.current_version, s.updated_at, s.deleted_at
...
pub async fn search(pool: &PgPool, q: &str) -> Result<Vec<NoteSummary>, AppError> {
    let rows = sqlx::query_as::<_, NoteSummary>(
        "select s.id, s.title, s.current_version, s.updated_at, s.deleted_at
         from note_streams s
...
async fn update_projection_and_stream(
    ...
    sqlx::query(
        "update note_projections set title = $2, version = $3, markdown = $4, metadata_json = $5, tags = $6,
         search_vector = to_tsvector('simple', $2 || ' ' || $4), updated_at = now() where note_id = $1",
    )
    .bind(note_id)
    .bind(title)
    .bind(version)
...
mod tests {
    ...
    async fn stale_patch_returns_version_conflict(pool: PgPool) -> anyhow::Result<()> {
        ...
        let note = create_note(&pool, actor, "conflict note", "hello").await?;
    ...
    async fn duplicate_idempotency_is_not_double_applied(pool: PgPool) -> anyhow::Result<()> {
        let actor = Uuid::now_v7();
        let note = create_note(&pool, actor, "idempotent note", "abc").await?;
        let ops = vec![
    ...
    async fn snapshot_created_every_100_versions(pool: PgPool) -> anyhow::Result<()> {
        let actor = Uuid::now_v7();
        let note = create_note(&pool, actor, "snapshot note", "x").await?;
