...
impl WsSession {
    ...
    fn subscribe(&mut self, note_id: Uuid, ctx: &mut ws::WebsocketContext<Self>) {
        if self.subscriptions.contains_key(&note_id) {
            return;
        }
        let addr: Addr<Self> = ctx.address();
        let state = self.state.clone();
        let handle = tokio::spawn(async move {
            let sender = state.topic_sender(note_id).await;
        ...
        self.subscriptions.insert(note_id, handle);
    ...
    fn unsubscribe(&mut self, note_id: Uuid) {
        if let Some(handle) = self.subscriptions.remove(&note_id) {
            handle.abort();
        }
    }
    ...
    fn handle_client_msg(&mut self, msg: ClientMessage, ctx: &mut ws::WebsocketContext<Self>) {
        match msg {
            ClientMessage::SubscribeNote { note_id } => self.subscribe(note_id, ctx),
            ClientMessage::UnsubscribeNote { note_id } => self.unsubscribe(note_id),
            ClientMessage::Ack { note_id, event_seq } => {
                self.ack_cursor.insert(note_id, event_seq);
                let addr = ctx.address();
                let pool = self.state.pool.clone();
                tokio::spawn(async move {
                ...
                tokio::spawn(async move {
                    ...
                    match result {
                        ...
                        Err(AppError::Conflict(msg)) => {
                            addr.do_send(SendText(
                                json!({"type": "error", "code": "PATCH_CONFLICT", "message": msg})
                                    .to_string(),
                            ));
                        }
                        Err(err) => {
                            addr.do_send(SendText(
                                json!({"type": "error", "code": "PATCH_ERROR", "message": err.to_string()}).to_string(),
                            ));
                        }
                    }
                });
        ...
        match item {
            Ok(ws::Message::Text(text)) => match serde_json::from_str::<ClientMessage>(&text) {
                ...
                Err(_) => Self::send_json(
                    ctx,
                    json!({"type": "error", "code": "BAD_MESSAGE", "message": "invalid websocket message"}),
                ),
            },
            ...
            Ok(ws::Message::Binary(_)) => Self::send_json(
                ctx,
                json!({"type": "error", "code": "BAD_MESSAGE", "message": "binary not supported"}),
            ),
