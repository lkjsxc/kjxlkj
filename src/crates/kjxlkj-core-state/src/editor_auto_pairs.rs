...
impl EditorState {
    ...
    pub(crate) fn try_auto_pair(&mut self, ch: char) -> bool {
        ...
        if is_quote(ch) {
            ...
            if col > 0 {
                if let Some(buf) = self.active_buffer() {
                    ...
                    let prev = line_text.chars().nth(col.saturating_sub(1));
                    if prev.map_or(false, |c| c.is_alphanumeric() || c == '_') {
                        return false;
                    }
                }
            }
    ...
    pub(crate) fn try_auto_pair_skip(&mut self, ch: char) -> bool {
        if !is_closing(ch) {
            return false;
        }
        let (line, col) = self.cursor_pos();
