...
impl EditorState {
    ...
    pub(crate) fn do_put(&mut self, before: bool) {
        let text = self.register_file.unnamed()
        ...
        if let Some(buf) = self.active_buffer_mut() {
            ...
            let insert_at = if before { off } else { off + 1 };
            let insert_at = insert_at.min(buf.content.len_chars());
            for (i, ch) in text.chars().enumerate() {
                buf.content.insert_char(insert_at + i, ch);
            }
            buf.modified = true;
        }
    ...
    /// Insert register contents at cursor (insert mode Ctrl-R).
    pub(crate) fn do_insert_register(
        &mut self,
        reg: char,
    ) {
        let name = crate::register_name_from_char(reg);
        let text = name.and_then(|n| {
            self.register_file.get(n)
                .map(|r| r.content.clone())
        }).unwrap_or_default();
        if text.is_empty() {
            return;
