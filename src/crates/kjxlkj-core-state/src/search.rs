...
impl SearchState {
    ...
    pub fn search(&mut self, pattern: &str, content: &BufferContent, forward: bool) {
        ...
        for line_idx in 0..content.line_count() {
            ...
            while let Some(pos) = line_lower[start..].find(&pat_lower) {
                ...
                let grapheme_offset = char_byte_to_grapheme(&line, byte_pos);
                self.matches.push((line_idx, grapheme_offset));
    ...
    pub fn next_match(&mut self, cursor: &CursorPosition) -> Option<CursorPosition> {
        if self.matches.is_empty() {
            return None;
        }
        let pos = (cursor.line, cursor.grapheme_offset);
        let idx = if self.forward {
            self.matches.iter().position(|m| *m > pos).unwrap_or(0)
        } else {
            self.matches
                .iter()
                .rposition(|m| *m < pos)
    ...
    pub fn prev_match(&mut self, cursor: &CursorPosition) -> Option<CursorPosition> {
        if self.matches.is_empty() {
            return None;
        }
        let pos = (cursor.line, cursor.grapheme_offset);
...
/// cluster index.
fn char_byte_to_grapheme(s: &str, byte_offset: usize) -> usize {
...
mod tests {
    ...
    #[test]
    fn empty_pattern() {
        let content = BufferContent::from_str("test\n");
        let mut state = SearchState::new();
