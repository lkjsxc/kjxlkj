...
impl EditorState {
    ...
    pub(crate) fn do_new_split(&mut self) {
        ...
        let mut win = crate::WindowState::new_buffer(win_id, buf_id);
    ...
    pub(crate) fn do_resize_cmd(&mut self, args: &str) {
        let args = args.trim();
        let (vertical, spec) = if let Some(rest) = args.strip_prefix("v ") {
            (true, rest.trim())
        } else {
            (false, args)
        };
        ...
        let (relative, value) = if let Some(rest) = spec.strip_prefix('+') {
        ...
        if vertical {
            if relative {
                self.do_resize_window(Direction::Right, value);
            } else if let Some(w) = self.focused_window_mut() {
                w.viewport.width = value.max(1) as u16;
            }
        } else if relative {
            self.do_resize_window(Direction::Down, value);
        } else if let Some(w) = self.focused_window_mut() {
...
mod tests {
    ...
    fn exchange_window() {
        let mut ed = EditorState::new(80, 24);
        ed.do_split_horizontal();
        let ids: Vec<_> = ed.windows.keys().copied().collect();
        let c0 = ed.windows[&ids[0]].content;
        let c1 = ed.windows[&ids[1]].content;
        ed.do_exchange_window();
        assert_eq!(ed.windows[&ids[0]].content, c1);
        assert_eq!(ed.windows[&ids[1]].content, c0);
    }
    ...
    fn resize_cmd() {
        let mut ed = EditorState::new(80, 24);
        ed.do_resize_cmd("10");
        assert_eq!(ed.focused_window().unwrap().viewport.height, 10);
    }
}
