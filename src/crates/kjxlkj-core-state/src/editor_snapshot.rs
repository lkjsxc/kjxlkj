...
use kjxlkj_core_ui::{
    BufferSnapshot, CmdlineState, EditorSnapshot, Rect, VisualRange, WindowLayout,
};
...
impl EditorState {
    ...
    pub fn snapshot(&mut self) -> EditorSnapshot {
        ...
        let visual = self.visual_state.as_ref().map(|vs| {
            ...
            let (start_line, start_col, end_line, end_col) = if (al, ac) <= (cl, cc) {
                (al, ac, cl, cc)
            } else {
                (cl, cc, al, ac)
            };
    ...
    fn build_buffer_snapshot(&self, buf: &BufferState) -> BufferSnapshot {
        let win = self.windows.get(&self.focused_window);
        let (cursor_line, cursor_col) = win
            .map(|w| (w.cursor.line, w.cursor.grapheme_offset))
            .unwrap_or((0, 0));
        let top_line = win.map(|w| w.viewport.top_line).unwrap_or(0);

        let height = win.map(|w| w.viewport.height as usize).unwrap_or(24);
