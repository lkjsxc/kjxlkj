...
impl EditorState {
    pub(crate) fn delete_lines(&mut self, count: usize) {
        ...
        if let Some(buf) = self.buffers.get_mut(buf_id) {
            ...
            let end_line =
                (start_line + count).min(buf.content.len_lines());
            ...
            if start_byte < end_byte {
                ...
                let yanked =
                    buf.content.slice(start_char..end_char).to_string();
        ...
        if let Some(reg) = self.registers.get_unnamed().cloned() {
            if reg.linewise {
                ...
                if let Some(buf) = self.buffers.get_mut(buf_id) {
                    ...
                    let byte_pos =
                        if next_line < buf.content.len_lines() {
                            buf.content.line_to_byte(next_line)
                        } else {
                            buf.content.len_bytes()
                        };
                    let char_idx = buf.content.byte_to_char(byte_pos);
                    let text = if byte_pos == buf.content.len_bytes()
                        && !reg.content.starts_with('\n')
                    {
                        format!(
                            "\n{}",
        ...
        if let Some(reg) = self.registers.get_unnamed().cloned() {
            if reg.linewise {
                ...
                if let Some(buf) = self.buffers.get_mut(buf_id) {
                    buf.save_undo_checkpoint(cursor);
                    let byte_pos =
                        buf.content.line_to_byte(cursor.line);
                    let char_idx = buf.content.byte_to_char(byte_pos);
