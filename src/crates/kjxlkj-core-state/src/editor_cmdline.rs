...
const COMMANDS: &[&str] = &[
    "buffer", "bdelete", "bnext", "bprevious",
    "close", "cquit",
    "edit", "find",
    "global",
    "help", "history",
...
impl EditorState {
    ...
    pub(crate) fn do_cmdline_history(
        &mut self,
        dir: Direction,
    ) {
        if let Some(cs) = &mut self.command_state {
...
fn longest_common_prefix(strs: &[&str]) -> String {
    ...
    for s in &strs[1..] {
        len = len.min(s.len());
        for (i, (a, b)) in
            first.bytes().zip(s.bytes()).enumerate()
        {
            if a != b {
                len = len.min(i);
                break;
...
mod tests {
    ...
    fn longest_common_prefix_works() {
        assert_eq!(
            longest_common_prefix(&["write", "wq", "wqa"]),
            "w"
        );
        assert_eq!(
            longest_common_prefix(&["split", "substitute"]),
            "s"
        );
    }
}
