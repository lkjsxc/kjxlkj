...
use kjxlkj_core_edit::{resolve_text_object, CursorPosition};
...
impl EditorState {
    ...
    pub(crate) fn do_operator_text_object(
        ...
        let range = {
            ...
            resolve_text_object(text_obj, &cursor, &buf.content, count)
        };
        ...
        if let Some(buf) = self.buffers.get_mut(&bid) {
            let s = buf
                .content
                .line_grapheme_to_offset(range.start.line, range.start.grapheme_offset);
            let e = buf
                .content
                .line_grapheme_to_offset(range.end.line, range.end.grapheme_offset);
    ...
    pub(crate) fn do_switch_buffer(&mut self, name: &str) {
    ...
    pub(crate) fn do_paste_text(&mut self, text: &str) {
