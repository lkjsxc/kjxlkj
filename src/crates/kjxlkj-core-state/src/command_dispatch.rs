...
fn dispatch_name(
    ...
    match name {
        ...
        "s" | "substitute" => {
            Some(Action::Substitute(args.into()))
        }
        "g" | "global" => {
            Some(Action::GlobalCommand(args.into()))
        }
        "v" | "vglobal" => {
            Some(Action::VglobalCommand(args.into()))
        }
        "sort" => {
            Some(Action::SortLines(args.into()))
        }
