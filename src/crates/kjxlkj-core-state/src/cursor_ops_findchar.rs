//! f/t/F/T motions: find char on current line.
//! Also toggle-case (~), lowercase/uppercase line helpers.
...
impl EditorState {
    /// f{char}: move forward to char on current line.
    pub(crate) fn find_char_forward(&mut self, ch: char) {
        self.last_ft = Some(('f', ch));
        self.apply_findchar_motion(Motion::FindCharForward(ch));
    }

    /// F{char}: move backward to char on current line.
    pub(crate) fn find_char_backward(&mut self, ch: char) {
        self.last_ft = Some(('F', ch));
        self.apply_findchar_motion(Motion::FindCharBackward(ch));
    }

    /// t{char}: move forward till (before) char.
    pub(crate) fn till_char_forward(&mut self, ch: char) {
        self.last_ft = Some(('t', ch));
        self.apply_findchar_motion(Motion::TillCharForward(ch));
    }

    /// T{char}: move backward till (after) char.
    pub(crate) fn till_char_backward(&mut self, ch: char) {
        self.last_ft = Some(('T', ch));
        self.apply_findchar_motion(Motion::TillCharBackward(ch));
    }
    ...
    pub(crate) fn repeat_find_char(&mut self) {
        if let Some((kind, ch)) = self.last_ft {
            let m = match kind {
                'f' => Motion::FindCharForward(ch),
                'F' => Motion::FindCharBackward(ch),
                't' => Motion::TillCharForward(ch),
                'T' => Motion::TillCharBackward(ch),
                _ => return,
            };
    ...
    pub(crate) fn toggle_case(&mut self) {
        ...
        if let Some(buf) = self.buffers.get_mut(buf_id) {
            ...
            let line_s: String = buf.content.line(line).chars().collect();
            ...
            let toggled: String = if chars[g].is_uppercase() {
                chars[g].to_lowercase().collect()
            } else {
                chars[g].to_uppercase().collect()
            };
            // Replace the single char.
            let byte_start = buf.content.line_to_byte(line)
                + chars[..g].iter().map(|c| c.len_utf8()).sum::<usize>();
            let byte_end = byte_start + chars[g].len_utf8();
            let cs = buf.content.byte_to_char(byte_start);
            let ce = buf.content.byte_to_char(byte_end);
            buf.content.remove(cs..ce);
            buf.content.insert(cs, &toggled);
    ...
    pub(crate) fn lowercase_lines(&mut self, start: usize, end: usize) {
        self.case_transform_lines(start, end, false);
    }

    /// Uppercase lines range (for gU operator linewise).
    pub(crate) fn uppercase_lines(&mut self, start: usize, end: usize) {
        self.case_transform_lines(start, end, true);
    }

    fn case_transform_lines(&mut self, start: usize, end: usize, upper: bool) {
        let buf_id = self.current_buffer_id();
        let cursor = self.windows.focused().cursor;
        if let Some(buf) = self.buffers.get_mut(buf_id) {
            buf.save_undo_checkpoint(cursor);
            for line_idx in start..=end.min(buf.content.len_lines().saturating_sub(1)) {
                let s: String = buf.content.line(line_idx).chars().collect();
                let new_s = if upper {
                    s.to_uppercase()
                } else {
                    s.to_lowercase()
    ...
    pub(crate) fn case_range(
        ...
        if let Some(buf) = self.buffers.get_mut(buf_id) {
            ...
            let ec = buf.content.byte_to_char(eb);
            buf.content.remove(sc..ec);
            buf.content.insert(sc, &new_text);
            buf.increment_version();
        }
