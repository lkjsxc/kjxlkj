...
fn dispatch_enter_visual() {
    let mut ed = editor();
    ed.dispatch(Action::EnterVisual(VisualKind::Char));
...
fn dispatch_replace_char() {
    ...
    let buf = ed.active_buffer().unwrap();
    let line = buf.content.line_content(0);
    assert!(line.starts_with('Z'), "expected 'Z' start, got: {:?}", line);
}
#[test]
fn dispatch_open_line_below() {
    let mut ed = editor();
    ed.dispatch(Action::EnterInsert(InsertPosition::NewLineBelow));
    assert_eq!(ed.mode, Mode::Insert);
...
fn dispatch_double_operator_delete() {
    ...
    ed.dispatch(Action::DoubleOperator(Operator::Delete, 1));
    let buf = ed.active_buffer().unwrap();
...
fn dispatch_toggle_case() {
    ...
    let buf = ed.active_buffer().unwrap();
    let line = buf.content.line_content(0);
    assert!(line.starts_with('A'), "expected 'A' start, got: {:?}", line);
}
#[test]
