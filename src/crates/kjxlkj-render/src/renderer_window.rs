...
use kjxlkj_core_ui::{BufferSnapshot, EditorSnapshot, Rect};
...
pub(crate) fn render_window(
    ...
    let is_focused = snapshot.layout.focused == win_id;

    if let Some((_id, buf)) = snapshot.buffers.iter().next() {
        ...
        render_buffer_text(grid, buf, text_x, rect.y, text_width, text_height);

        if is_focused {
            let cursor_row = buf.cursor_line.saturating_sub(buf.top_line) as u16;
            let cursor_col = text_x + buf.cursor_col as u16;
            if cursor_row < text_height && cursor_col < rect.x + rect.width {
                let cell = grid.get_mut(cursor_col, rect.y + cursor_row);
