...
pub fn render_notification(
    text: &str, position: NotifPosition,
    term_w: u16, term_h: u16, max_width: usize,
) -> RenderedNotif {
    ...
    let col = match position {
        NotifPosition::TopRight | NotifPosition::BottomRight => {
            term_w.saturating_sub(notif_w + 1)
        }
    ...
    let lines = wrapped.into_iter().map(|l| {
        let mut s = l;
        if (s.len() as u16) < notif_w {
            s.extend(std::iter::repeat(' ').take(notif_w as usize - s.len()));
        }
        s
    }).collect();
